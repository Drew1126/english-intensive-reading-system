...
补充规则（严格遵守）：
- 形近词 = 与目标词**拼写相近、且是不同的常用单词**，容易看错或记混（如 native↔naive、affect↔effect、principal↔principle）。不限只差一个字母。
- **禁止把目标词自身列出来**（如查 justification 就不要列 justification）。
- **禁止编造不存在的单词、拼写变体或"无此词"之类的自我解释**。只列真实存在的常用词。
- 同根/同族词（同一词根的不同词性，如 justification / justify / justified / justifiable）**不属于形近词**，不要放进形近词一节；可在近义词/同义替换或考研提示中简单说明词性变化。
- 近义词 / 同义替换 = 意思相近易混的词，或考研中可同义替换的词（如 nuts↔crazy/insane/absurd；justification 可列 reason / grounds / rationale 等）。
- 每个词都需给出中文意思，并一句话说明区分点或替换关系。
- **某类确实没有时，直接整体省略该节（连同其标题行），不要硬凑**，也不要写任何"省略""无此词""本节省略"之类的说明文字。例如：justification 没有真正的形近词，就完全不输出"◆ 形近词"这一节。
